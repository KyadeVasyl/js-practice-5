...
function addTenToEach(array) {
  // Ініціалізуємо масив через конструктор з такою же
  // кількістю елементів як в вхідному масиві для зберігання результату
  let length = array.length;
  let result = [length];
  // Перебираємо масив за допомогою циклу for-in
  for (let element in array) {
    result[element] = array[element] + 10;
  }
...
function swapMinMax(array) {
  ...
  // Ці змінні будуть використовуватися для зберігання максимального та мінімального значень в масиві.
  let min = array[0];
  let max = array[0];
  ...
  // Починаємо з першого елемента(i = 0) і продовжуємо до останнього елемента масиву(i < array.length).
  for (let i = 0; i < array.length; i++) {
    if (array[i] > max) {
      max = array[i];
      maxIndex = i;
...
// Завдання 13: Функція приймає масив чисел. Повертає новий масив, в якому всі числа більші за 10 замінені 
//на рядок "Greater than 10", а решта чисел замінені на рядок "Less than or equal to 10".
function replaceNumbers(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] > 10) {
      result[index] = "Greater than 10";


    } else if (arr[i] <= 10) {
      result[index] = "Less than or equal to 10";
    } index++;
  } return result;
